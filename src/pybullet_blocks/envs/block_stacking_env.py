"""A version of the classic block stacking problem."""
...
class BlockState:
    """The state of a single block in the BlockStackingPyBulletBlocksEnv."""
    ...
    def from_vec(cls, vec: NDArray[np.float32]) -> BlockState:
        ...
        _, letter_vec, held_vec, pos_vec, orn_vec = np.split(
            vec, [1, 2, 3, 6]
        )
...
@dataclass(frozen=True)
class RobotState:
    """The state of a robot in the BlockStackingPyBulletBlocksEnv."""
    ...
    def from_vec(cls, vec: NDArray[np.float32]) -> BlockState:
        """Build a state from a vector."""
        _, robot_joints_vec, grasp_vec = np.split(vec, [1, 9])
...
@dataclass(frozen=True)
class BlockStackingPyBulletBlocksState:
    """A state in the BlockStackingPyBulletBlocksEnv."""

    robot_state: RobotState
    block_states: list[BlockState]

    def to_graph(self) -> spaces.GraphInstance:
        """Create graph representation of the state."""
        graph_dim = 1 + 7 + 8  # max of robot and block dim
        robot_vec = self.robot_state.to_vec()
        block_vecs = [b.to_vec() for b in self.block_states]
        vecs = [robot_vec] + block_vecs
        padded_vecs = []
        for vec in vecs:
            assert len(vec) <= graph_dim
            padded_vec = np.zeros((graph_dim,), dtype=np.float32)
            padded_vec[:len(vec)] = vec
            padded_vecs.append(padded_vec)
        nodes = np.array(padded_vecs, dtype=np.float32)
        return spaces.GraphInstance(nodes=nodes, edges=None, edge_links=None)

    @classmethod
    def from_graph(cls, graph: spaces.GraphInstance) -> BlockStackingPyBulletBlocksState:
        """Build a state from a vector."""
        robot_state: RobotState | None = None
        block_states: list[BlockState] = []
        for node in graph.nodes:
            if np.isclose(node[0]):  # robot
                assert robot_state is None
                robot_state = RobotState.from_vec(node)
            else:
                assert np.isclose(node[0])  # block
                block_state = BlockState.from_vec(node)
                block_states.append(block_state)
        return cls(robot_state, block_states)



@dataclass(frozen=True)
class BlockStackingPyBulletBlocksAction:
    """An action in the BlockStackingPyBulletBlocksAction."""
    ...
    @classmethod
    def from_vec(cls, vec: NDArray[np.float32]) -> BlockStackingPyBulletBlocksAction:
